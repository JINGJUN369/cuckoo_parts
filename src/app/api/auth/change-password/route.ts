...
export async function POST(request: NextRequest) {
  try {
    ...
    // === 비밀번호 초기화 (관리자 전용) ===
    if (action === 'reset') {
      const { adminUserCode, targetUserCode } = body;
      ...
      const resetPassword = isAdminAccount ? ADMIN_DEFAULT_PASSWORD : targetUserCode;
      const hashedResetPw = await bcrypt.hash(resetPassword, SALT_ROUNDS);
    ...
    // === 비밀번호 변경 (본인) ===
    const { userCode, newPassword } = body;
    ...
    console.error('Password change/reset error:', error);
