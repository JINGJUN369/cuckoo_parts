          ...
          <Tabs defaultValue="upload">
            <TabsList>
              ...
              <TabsTrigger value="selection" className="flex items-center gap-2">
                <CheckSquare className="h-4 w-4" />회수대상 선택
                {productStats.unselected > 0 && (
                  <Badge variant="secondary" className="ml-1">{productStats.unselected}</Badge>
                )}
              </TabsTrigger>
            </TabsList>
                ...
                <CardHeader>
                  <div className="flex flex-col sm:flex-row justify-between gap-4">
                    <div>
                      ...
                      <CardDescription>자동선택 조건에 해당하지 않아 수동으로 검토가 필요한 항목입니다.</CardDescription>
                    </div>
                    <div className="flex gap-2">
                      <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                        <Input placeholder="검색..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 w-[200px]" />
                      </div>
                      <Select value={modelFilter} onValueChange={setModelFilter}>
                        <SelectTrigger className="w-[180px]"><SelectValue placeholder="모델 필터" /></SelectTrigger>
                        <SelectContent>
                ...
                <CardContent>
                  {filteredUnselected.length > 0 ? (
                        ...
                        <TableHeader>
                          <TableRow>
                            <TableHead className="w-[50px]">
                              <Checkbox checked={selectedIds.size === filteredUnselected.length && filteredUnselected.length > 0} onCheckedChange={handleSelectAll} />
                            </TableHead>
                        ...
                        <TableBody>
                          {filteredUnselected.slice(0, 100).map((item) => (
                            <TableRow key={item.id} className={selectedIds.has(item.id) ? 'bg-blue-50' : ''}>
                              ...
                              <TableCell><Badge variant={item.recovery_type === '철거' ? 'default' : 'secondary'}>{item.recovery_type}</Badge></TableCell>
                              <TableCell className="font-mono text-sm">{item.customer_number}</TableCell>
                              <TableCell>{item.customer_name}</TableCell>
                              <TableCell>
                                {item.model_name}
                                {item.is_auto_recovery_model && <Badge variant="outline" className="ml-1 text-xs">회수모델</Badge>}
                              </TableCell>
                              ...
                              <TableCell className="text-sm">
                                ...
                                {item.is_within_one_year && <Badge variant="outline" className="ml-1 text-xs text-orange-600">1년이내</Badge>}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                    ...
                    <div className="text-center py-12 text-muted-foreground">
                      {unselectedData.length === 0 ? '미선택 항목이 없습니다.' : '검색 결과가 없습니다.'}
                    </div>
                  )}
                  {filteredUnselected.length > 100 && (
                    <p className="text-sm text-muted-foreground mt-4 text-center">처음 100건만 표시됩니다. 검색을 활용해주세요.</p>
                  )}
                </CardContent>
              </Card>
            </TabsContent>
